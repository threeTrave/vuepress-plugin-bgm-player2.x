<template>
  <div class="reco-bgm-panel">
    ...
    <module-transition>
      <div
        ...
        v-show="!isFloat"
        :style="panelPosition"
        @mouseenter="isBgmBoxVisible = true"
        @mouseleave="isBgmBoxVisible = false"
        @blur="isBgmBoxVisible = false"
        ref="reco-bgm-box"
      >
        <!-- 封面 -->
        <div
        ...
        >
          ...
          <div v-show="isMini" class="mini-operation">
            <i
              v-show="curPlayStatus === 'playing' && isMini"
              @click.stop="pauseBgm"
              class="reco-bgm reco-bgm-pause"
            ></i>
            <i
              v-show="curPlayStatus === 'paused' && isMini"
              @click.stop="playBgm"
          ...
          <div v-show="!isMini" class="reco-bgm-info">
            <transition name="opacity" mode="out-in">
              ...
              <div v-else class="reco-bgm-operation">
                <!-- 用来扩充info box 的长度 -->
                <div
                  class="reco-bgm-cover-mask"
                  @mouseleave="isDurationVisible = false"
                  @mouseenter="isDurationVisible = true"
                >
                  <!-- 时长 -->
                  <transition name="time">
                    ...
                    <div
                      v-else
                      class="reco-bgm-time"
                      @click="isDurationVisible = false"
                    >
                      {{
                        `${parseInt(
                          ($refs.bgm ? $refs.bgm.duration : 0) / 60
                        )}`.padStart(2, "0") +
                        ":" +
                        `${parseInt(
                          ($refs.bgm ? $refs.bgm.duration : 0) % 60
                ...
                <i
                  ...
                  @mouseleave="isVolumeBarVisible = false"
                  class="reco-bgm reco-bgm-volume1 volume"
                >
                  <div class="hoverBar" v-show="isVolumeBarVisible">
                ...
                <i
                  class="reco-bgm-add reco-bgm-add-lyric"
                  @click="isLyricVisible = !isLyricVisible"
                  v-show="!isMobile"
                >
                </i>
              </div>
  ...
  <Lyric
    :value="currentTime"
    :lrc="audio[curIndex].lyric || '[00:00.000] 暂无歌词，请欣赏'"
    v-show="isLyricVisible"
    ref="lyric"
  ></Lyric>
</template>
